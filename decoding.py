...
class Abstractor(object):
    def __init__(self, abs_dir, max_len=30, cuda=True, min_len=0, reverse=True):
        ...
        if self._bert:
            ...
            self._end = self._tokenizer.encoder[self._tokenizer.eos_token]
            self._unk = self._tokenizer.encoder[self._tokenizer.unk_token]
        else:
            self._end = END
            self._unk = UNK
    ...
    def _prepro(self, raw_article_sents):
        if self._bert:
            ...
            unk = word2id[self._tokenizer.unk_token]
            start = self._tokenizer.encoder[self._tokenizer.bos_token]
            end = self._tokenizer.encoder[self._tokenizer.eos_token]
            pad = self._tokenizer.encoder[self._tokenizer.pad_token]
...
class BeamAbstractorGAT(object):
    def __init__(self, abs_dir, max_len=100, cuda=True, min_len=0, reverse=True, key='summary_worthy', docgraph=True):
        ...
        if self._bert:
            ...
            self._tokenizer = abstractor._bert_model._tokenizer
            try:
                with open(r'E:\proj\p1\acl2020_cnndata\finished_files_openie_3\roberta-base-align.pkl', 'rb') as f:
